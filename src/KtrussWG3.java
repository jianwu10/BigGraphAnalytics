...
*  This implementation removes edgeTail array.
*/
...
public class KtrussWG3 {
  final ImmutableGraph G; // WebGraph object
  final ImmutableGraph G_pos;
  ...
  public KtrussWG3(String basename) throws Exception {
    G = ImmutableGraph.loadMapped(basename);
    G_pos = ImmutableGraph.loadMapped(basename+"-pos");
    n = G.numNodes();
    degree = new int[n];
    offset = new int[n+1];
    ...
    for (int u = 0; u < n; u++) {
      ...
      int[] u_successor = G_pos.successorArray(u);
      Arrays.sort(u_successor);
      int count = 0;
      for (int j = 0; j < u_successor.length; j++) {
        int v = u_successor[j];
        if (u <  v) {
          edgePos[startPos+count] = v;
          count++;
        }
      }
      startPos += count;
      ...
      Arrays.stream(index)
            ...
            .mapToInt(e -> {
              ...
              while (w1 < offset[u+1] && w2 < offset[v+1]) {
                if (edgePos[w1] < edgePos[w2]) {
                  w1++;
                } else if (edgePos[w1] > edgePos[w2]) {
                  w2++;
                } else {
                  // edgePos[w1] = edgePos[w2]
  ...
  public void ktrussCompute() {
    ...
    while (edgeRemoved < m) {
      ...
      while (edge < m && support.get(sortedEdge[edge]) <= k-2) {
        ...
        int[] u_neighbor_pos = G_pos.successorArray(u);
        int v = u_neighbor_pos[id_uv-offset[u]]; // edge's tail
        ...
        // cache u and v's pos neighbors
        u_neighbor_pos = G_pos.successorArray(u);
        int[] v_neighbor_pos = G_pos.successorArray(v);
        ...
        for (int w : G.successorArray(u)) {
          ...
          int[] targetArray_uw = new int[0];
          if (w < u) {
            targetArray_uw = G_pos.successorArray(w);
          } else {
            targetArray_uw = u_neighbor_pos;
          }
          int id_uw_offset = Arrays.binarySearch(targetArray_uw,Math.max(u,w));
          int id_uw = id_uw_offset + offset[Math.min(u,w)];
          // If <u,w> is an edge and has not been removed
          if (id_uw_offset >= 0 && !removed.get(id_uw)) {
            // Check if <v, w> is an edge or not
            // Check if <v, w> has been removed or not
            if (w != v) {
              int[] targetArray_vw = new int[0];
              if (u < v) {
                if (w < u) {
                  targetArray_vw = targetArray_uw;
                } else if (u < w && w < v) {
                  targetArray_vw = G_pos.successorArray(w);
                } else {
                  targetArray_vw = v_neighbor_pos;
                }
              } else {
                if (w < v) {
                  targetArray_vw = targetArray_uw;
              ...
              int id_vw_offset = Arrays.binarySearch(targetArray_vw,Math.max(v,w));
              int id_vw = id_vw_offset + offset[Math.min(v,w)];
              // If <v, w> is an edge and not removed
              if (id_vw_offset >= 0 && !removed.get(id_vw)) {
  ...
  public static void main(String[] args) throws Exception{
    ...
    // PrintStream out = new PrintStream(new FileOutputStream("data/"+basename+"/"+"output/ktruss/"+basename+"_ktruss_sequential3_console.txt"));
    // System.setOut(out);

    KtrussWG3 kt = new KtrussWG3("data/"+basename+"/"+basename+"-noself-sym");
    kt.supportCompute();
    kt.ktrussCompute();

    // System.out.println("Writing results to file. Be patient...");
    // PrintStream ps = new PrintStream(new File("data/"+basename+"/"+"output/ktruss/"+basename+"_ktruss_sequential3.txt"));
